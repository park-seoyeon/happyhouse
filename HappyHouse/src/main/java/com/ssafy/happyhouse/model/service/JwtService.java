...
//		Payload 설정 : 가장 중요하다. 넘겨줘야 하는 데이터가 무엇인지 설정
		jwtBuilder
			.setSubject("로그인토큰") // 토큰의 제목 설정
			.setExpiration(new Date(System.currentTimeMillis() + 1000 * 60 * expireMin)) // 유효기간 설정
			.claim("user", memberDto).claim("greeting", "환영합니다. " + memberDto.getUsername()); // 담고 싶은 정보 설정. JWT안에 포함되는 정보
		
//		signature 설정(암호화같은 느낌)
